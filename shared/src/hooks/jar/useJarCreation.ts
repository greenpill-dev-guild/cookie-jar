...
export function useJarCreation({
	...
	useEffect(() => {
		...
		onCreated(`/jar/${jarAddress}?chainId=${submission.chainId}`);
	}, [receiptQuery.data, submission, queryClient, onCreated, toast]);
	...
	return {
		...
		presetApplied,
		presetCustomized:
			presetApplied && JSON.stringify(values) !== JSON.stringify(preset),
		resetForm: () => {
			if (!busyRef.current) {
