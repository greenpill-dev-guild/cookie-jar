  ...
  for (const service of services) {
    ...
    while (attempts < maxAttempts) {
      ...
      if (attempts >= maxAttempts) {
        throw new Error(`❌ ${service.name} failed to start within 2 minutes. Make sure 'pnpm dev' is running.`)
      }
    }
  }
}
...
async function verifyContracts() {
  try {
    ...
    const response = await fetch('http://localhost:3000/contracts/local-deployment.json')
    
    if (!response.ok) {
      throw new Error('Deployment file not found. Ensure contracts are deployed with `pnpm dev`.')
    }
    ...
    console.log('   This will start Anvil, deploy contracts, and start the client')
    throw error
  }
}
