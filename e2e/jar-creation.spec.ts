...
test.describe('🏗️ Jar Creation E2E', () => {
  ...
  test('Create basic ETH jar with allowlist access', async ({ page, wallet }) => {
    ...
    await page.fill(SELECTORS.forms.jarDescription, 'Created by automated E2E test')
    ...
    await expect(page.locator(SELECTORS.cards.jarTitle)).toContainText('E2E Test Jar')
  ...
  test('Create NFT-gated jar (v2 contracts only)', async ({ page, wallet }) => {
    ...
    await page.fill(SELECTORS.forms.jarDescription, 'NFT-gated jar for testing')
    ...
    await expect(page.locator(SELECTORS.cards.jarTitle)).toContainText('NFT Test Jar')
    await expect(page.locator('text=NFT-Gated, text=NFT Collection')).toBeVisible()
