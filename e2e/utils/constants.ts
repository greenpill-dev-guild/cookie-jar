...
export const SELECTORS = {
  ...
  wallet: {
    ...
    connected:
      '[data-testid="account-modal-button"], [title*="0x"], text=/0x[a-fA-F0-9]{4}/',
  ...
  forms: {
    jarName:
      '[data-testid="jar-name-input"], input[placeholder*="name"], input:below(:text("Name"))',
    jarOwner: '[data-testid="jar-owner-input"], input:below(:text("Owner"))',
    jarDescription:
      'textarea[placeholder*="description"], input:below(:text("Description"))',
    currency:
      '[data-testid="currency-selector"], select:has(option[value*="ETH"]), [role="combobox"]:near(:text("Currency"))',
  ...
  buttons: {
    primary:
      '.cj-btn-primary, button:has-text("Create"), button:has-text("Deposit")',
  ...
  status: {
    ...
    success:
      'text=successful, text=created, text=updated, text=Cookie Jar Created',
  ...
  nft: {
    ...
    typeSelect:
      'select:has(option:text("ERC721")), [role="combobox"]:near(:text("Type"))',
