...
test.describe('🎨 NFT Gating Integration (v2 Contracts)', () => {
  test('Verify NFT-gated jars exist and display correctly', async ({
    page,
  }) => {
  ...
  test('NFT contract address validation in create form (v2 only)', async ({
    page,
  }) => {
    ...
    if (isNextEnabled) {
      ...
      if (
        (await nextButton2.count()) > 0 &&
        !(await nextButton2.first().isDisabled())
      ) {
