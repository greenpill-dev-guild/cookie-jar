...
test.describe('📱 Mobile Experience E2E', () => {
  ...
  test.describe('Mobile Navigation & UI', () => {
    test('Mobile homepage navigation works correctly', async ({ page }) => {
      ...
      const mobileMenuButton = page.locator('button[aria-label*="menu"], button:has([data-icon="menu"])')
      if (await mobileMenuButton.isVisible()) {
        await mobileMenuButton.click()
        
        // Should show navigation options
        await expect(page.locator('a[href="/create"], text=Create Jar')).toBeVisible()
        await expect(page.locator('a[href="/browse"], text=Browse Jars')).toBeVisible()
      }
    ...
    test('Mobile wallet connection experience', async ({ page, wallet }) => {
      ...
      await expect(page.locator('text=/0x[a-fA-F0-9]{4}/', 'button[title*="0x"]')).toBeVisible({ timeout: 10000 })
    })
  })
  ...
  test.describe('Mobile Jar Creation', () => {
    test('Mobile jar creation flow is intuitive', async ({ page, wallet }) => {
      ...
      const withdrawalOptions = page.locator('button:has-text("Fixed Amount"), button:has-text("Variable Amount")')
      ...
      await expect(page.locator('text=Choose Access Method, text=Access Control')).toBeVisible()
    })

    test('Mobile NFT protocol selector works correctly', async ({ page, wallet }) => {
      ...
      const protocolCards = page.locator('button:has-text("Allowlist"), button:has-text("NFT Collection"), button:has-text("POAP")')
      ...
      const nftConfig = page.locator('text=NFT Configuration, text=Select NFT Collections')
      if (await nftConfig.first().isVisible({ timeout: 10000 })) {
        // NFT search should be mobile-friendly
        const searchInput = page.locator('input[placeholder*="search"], input[placeholder*="NFT"]')
    ...
    test('Mobile form validation and error states', async ({ page, wallet }) => {
      ...
      const errorMessages = page.locator('text=required, text=field is required, .error-message')
      ...
      const validationFeedback = page.locator('text=too short, text=minimum, .validation-error')
  ...
  test.describe('Mobile Jar Viewing', () => {
    test('Mobile jar details view is optimized', async ({ page, wallet }) => {
      ...
      const detailsCards = page.locator('[data-testid="jar-details"], .jar-detail-card')
    ...
    test('Mobile withdrawal flow works correctly', async ({ page, wallet }) => {
      ...
      if (await withdrawTab.isVisible()) {
        ...
        const purposeInput = page.locator('input[placeholder*="purpose"], textarea[placeholder*="purpose"]')
        ...
        const withdrawButton = page.locator('button:has-text("Withdraw"), button:has-text("Get Cookie")')
    ...
    test('Mobile streaming interface (v2 jars)', async ({ page, wallet }) => {
      ...
      if (await streamingTab.isVisible({ timeout: 15000 })) {
        ...
        await expect(page.locator('text=Token Recovery, text=Stream Management')).toBeVisible()
        ...
        if (await recoveryTab.first().isVisible()) {
          ...
          const processButtons = page.locator('button:has-text("Process"), button:has-text("Swap")')
