...
test.describe('🍪 Complete Cookie Jar Workflow E2E', () => {
  test('End-to-end jar lifecycle - create, configure, fund, withdraw', async ({ page, wallet }) => {
    ...
    await page.fill(SELECTORS.forms.jarDescription, 'End-to-end test for complete jar lifecycle')
    ...
    const hasHistory = await page.locator('text=History, text=Withdrawal').isVisible()
    if (hasHistory) {
      await expect(page.locator('text=End-to-end test withdrawal')).toBeVisible()
    ...
    console.log('🔒 Step 6: Testing access control with non-allowlisted user...')
  ...
  test('NFT-gated jar complete workflow (v2 only)', async ({ page, wallet }) => {
    ...
    test.skip(!hasNFTOption, 'NFT gating not available - requires v2 contracts')
    ...
    const hasWithdrawAccess = await page.locator(SELECTORS.buttons.withdraw).isEnabled()
