...
test.describe('🌊 Streaming Features E2E', () => {
  ...
  test('Version detection - shows v1 fallback UI for legacy jars', async ({ page }) => {
    ...
    await expect(page.locator('a:has-text("Learn about upgrading")')).toBeVisible()
  ...
  test('v2 jar streaming workflow - token recovery and processing', async ({ page, wallet }) => {
    ...
    await page.fill(SELECTORS.forms.jarDescription, 'Testing v2 streaming and token recovery features')
    ...
    const streamingTab = page.locator('[data-testid="streaming-tab"], text=Streaming')
    ...
    const recoveryTab = page.locator('text=Recovery, [data-testid="recovery-tab"]').first()
    ...
    const streamingManagementTab = page.locator('text=Streams, [data-testid="streams-tab"]').first()
  ...
  test('Lazy loading - components load on demand', async ({ page, wallet }) => {
    ...
    const protocolSelectorLoader = page.locator('text=Loading protocol selector...')
    ...
    await expect(page.locator('text=Allowlist')).toBeVisible({ timeout: 10000 })
    await expect(page.locator('text=NFT Collection')).toBeVisible({ timeout: 5000 })
  })
  ...
  test('Mobile streaming experience', async ({ page, wallet }) => {
    ...
    await page.fill(SELECTORS.forms.jarDescription, 'Testing mobile streaming UI')
    ...
    if (await streamingTab.isVisible({ timeout: 15000 })) {
      ...
      const streamingContainer = page.locator('[data-testid="streaming-container"]').first()
    ...
    await expect(page.locator('text=Choose Access Method')).toBeVisible({ timeout: 10000 })
    
    // Protocol cards should be in mobile grid layout (3 columns)
    const protocolCards = page.locator('button:has-text("Allowlist"), button:has-text("NFT Collection")')
  ...
  test('Performance benchmarks - Core Web Vitals', async ({ page }) => {
    ...
    if (await streamingTab.isVisible({ timeout: 10000 })) {
      ...
      await expect(page.locator('text=How Streaming Works')).toBeVisible({ timeout: 5000 })
  ...
  test('Accessibility - streaming features are WCAG compliant', async ({ page }) => {
    ...
    let attempts = 0
    while (attempts < 20) { // Max 20 tab presses to find streaming tab
    ...
    const streamingContent = page.locator('text=Token Streams, text=Token Recovery').first()
    if (await streamingContent.isVisible({ timeout: 10000 })) {
      // Check for proper ARIA labels
      const streamingPanels = page.locator('[role="tabpanel"], [aria-label*="streaming"]')
      ...
      const textElements = page.locator('text=Token Streams, text=Token Recovery')
      if (await textElements.first().isVisible()) {
        const styles = await textElements.first().evaluate((el) => getComputedStyle(el))
  ...
  test('Error handling - graceful degradation', async ({ page }) => {
    ...
    await expect(page.locator('text=Invalid Address, text=No valid address')).toBeVisible({ timeout: 10000 })
    ...
    const errorMessages = page.locator('text=Error Loading, text=Unknown error, text=Failed to load')
    ...
    await expect(page.locator('text=Cookie Jar')).toBeVisible({ timeout: 5000 })
...
test.describe('🔧 Advanced Streaming Scenarios', () => {
  ...
  test('Real-time updates - streaming data refresh', async ({ page, wallet }) => {
