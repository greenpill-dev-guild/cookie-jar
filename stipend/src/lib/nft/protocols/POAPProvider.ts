...
export class POAPProvider {
	...
	constructor(apiKey?: string) {
		this.apiKey = apiKey || import.meta.env.VITE_POAP_API_KEY || "";
	}
	...
	async searchEvents(
		...
		try {
			...
			return {
				...
				nextPageParam:
					formattedEvents.length === limit ? String(offset + limit) : undefined,
			};
		} catch (error) {
	...
	async getUserPOAPs(
		...
		try {
			...
			return {
				...
				nextPageParam:
					formattedTokens.length === limit ? String(offset + limit) : undefined,
			};
		} catch (error) {
