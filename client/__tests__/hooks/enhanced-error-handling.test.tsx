...
describe("Enhanced Error Handling Tests", () => {
    ...
    it("handles race conditions during network switching", async () => {
      ...
      mockAlchemyProvider.getUserNFTs.mockImplementation(async () => {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, currentRequest * 100),
        );
  ...
  describe("useNFTMetadata Error Scenarios", () => {
    ...
    it("handles excessive metadata size", async () => {
      const largeMetadata = {
        ...
        traits: Array.from({ length: 1000 }, (_, i) => ({
          // Many traits
          trait_type: `trait_${i}`,
          value: `value_${i}`,
        })),
  ...
  describe("useEnhancedNFTValidation Error Scenarios", () => {
    it("handles malicious contract detection", async () => {
      ...
      const { result } = renderHook(
        () =>
          useEnhancedNFTValidation(
            "0xbadcontract123456789012345678901234567890",
          ),
        { wrapper: createWrapper() },
      );
    ...
    it("handles non-existent contracts", async () => {
      ...
      const { result } = renderHook(
        () =>
          useEnhancedNFTValidation(
            "0xnonexistent123456789012345678901234567890",
          ),
        { wrapper: createWrapper() },
      );
  ...
  describe("useNFTBalanceProof Error Scenarios", () => {
    it("handles stale balance proofs", async () => {
      const mockUseReadContract = require("wagmi")
      ...
      const mockUseBlockNumber = require("wagmi").useBlockNumber as ReturnType<
        typeof vi.fn
      >;
...
export const createErrorScenarios = () => {
  return {
    ...
    rateLimitError: () =>
      new Error("API rate limit exceeded. Please wait and try again."),
    timeoutError: () => new Error("Request timeout"),
    maliciousContractError: () => new Error("execution reverted"),
    invalidFormatError: () =>
      new Error("Invalid response format from NFT provider"),
    contractNotFoundError: () =>
      new Error("Unable to fetch contract metadata - contract may not exist"),
  };
};
