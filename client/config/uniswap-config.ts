 ...
 */
export function isUniversalRouterAvailable(chainId: number): chainId is SupportedChain {
...
export function getWrappedNative(chainId: number): Address | null {
  return WRAPPED_NATIVE_ADDRESSES[chainId as keyof typeof WRAPPED_NATIVE_ADDRESSES] || null;
}

/**
 * Get default multi-token configuration for Universal Router
 */
export function getDefaultMultiTokenConfig(chainId: number): UniversalMultiTokenConfig {
  return {
...
export function getChainCapabilities(chainId: number): ChainCapabilities {
  ...
  return {
    ...
    supportedVersions: hasUniversalRouter 
      ? ['v2', 'v3', 'v4'] 
      : [],  // Universal Router supports all versions
  };
}
...
export function isLikelyStablePair(tokenA: Address, tokenB: Address): boolean {
  ...
  return stablecoins.includes(tokenA.toLowerCase() as Address) || 
