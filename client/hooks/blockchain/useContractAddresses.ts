  ...
  useEffect(() => {
    ...
    if (chainId === 31337) {
      const loadLocalDeployment = async () => {
        dev("Loading local deployment for anvil chain", undefined, "useContractAddresses");
        ...
        try {
          ...
          if (!response.ok) {
            dev("HTTP fetch failed", { status: response.status, statusText: response.statusText }, "useContractAddresses");
          ...
          if (deploymentTime > lastDeploymentTime || !localFactory) {
            dev("Updating local factory address", {
            ...
            dev("Skipping update (no newer deployment)", {
              deploymentTime,
              lastDeploymentTime,
          ...
          logError("Error loading local deployment", error, "useContractAddresses");

          // Fallback to static addresses for local development
          const fallbackAddress = staticAddresses.cookieJarFactory[31337];
          dev("Falling back to static address", { fallbackAddress }, "useContractAddresses");
