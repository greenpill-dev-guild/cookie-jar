...
export const createSuperfluidFramework = async (chainId: number) => {
  ...
  const providerUrl = chainId === 31337
    ? "http://localhost:8545"
    : rpcUrl.replace("YOUR_INFURA_KEY", process.env.NEXT_PUBLIC_INFURA_KEY || "");
 ...
 */
export const isSuperfluidSupported = (chainId: number): chainId is SupportedSuperfluidChain => {
 ...
 */
export const getSuperfluidTokenAddress = (chainId: number, symbol: "ETHx" | "USDCx" | "DAIx" = "ETHx"): string | null => {
  if (symbol === "ETHx") {
