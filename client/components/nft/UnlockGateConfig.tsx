...
import type { LockInfo } from "@/hooks/nft/useUnlock";
...
export const UnlockGateConfig: React.FC<UnlockGateConfigProps> = ({
  ...
  const { lockInfo, isLoadingLock, lockError, validateLockAddress } =
    useUnlock({
      lockAddress:
  ...
  const getValidationIcon = () => {
    ...
    if (lockError || (debouncedAddress && !isAddress(debouncedAddress)))
      ...
      <div className="space-y-4">
          ...
          <Card className="border-l-4 border-l-[#ff5e14]">
            ...
            <CardContent className="pt-0">
              <div className="grid grid-cols-2 gap-4">
                ...
                {lockInfo.totalSupply !== undefined &&
                  lockInfo.maxNumberOfKeys && (
                    <div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="text-xs font-mono text-[#3c2a14] bg-gray-100 px-2 py-1 rounded">
                      {lockInfo.address.slice(0, 10)}...
                      {lockInfo.address.slice(-8)}
                    </code>
                    <Button
                      variant="ghost"
                      size="sm"
                      ...
                      onClick={() =>
                        window.open(
                          `https://etherscan.io/address/${lockInfo.address}`,
                          "_blank",
                        )
                      }
                    >
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium text-[#3c2a14] mb-2">
            How Unlock Protocol Gating Works
          </h4>
          <ul className="text-xs text-[#8b7355] space-y-1 list-disc list-inside">
            <li>
              Only holders of active (non-expired) keys from the specified lock
              can access the jar
            </li>
          ...
          <div className="mt-3 pt-3 border-t">
            ...
            <p className="text-xs text-[#8b7355]">
              The contract verifies membership by calling getHasValidKey() on
              the lock contract, which checks both ownership and expiration
              status.
            </p>
          </div>
        </div>
