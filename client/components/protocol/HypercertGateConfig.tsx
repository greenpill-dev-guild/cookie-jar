...
export const HypercertGateConfig: React.FC<HypercertGateConfigProps> = ({
  ...
  const [tokenContract, setTokenContract] = useState(
    initialConfig?.tokenContract || "",
  );
  const [tokenId, setTokenId] = useState(initialConfig?.tokenId || "");
  const [minBalance, setMinBalance] = useState(
    initialConfig?.minBalance || "1",
  );
  const [hypercertInfo, setHypercertInfo] = useState<HypercertInfo | null>(
    initialConfig?.hypercertInfo || null,
  );
  ...
  const getValidationIcon = () => {
    ...
    if (hypercertInfo && !validationError)
    ...
    <div className={`space-y-6 ${className}`}>
      <div>
        <Label className="text-[#3c2a14] text-base font-semibold">
          Hypercert Configuration
        </Label>
        <p className="text-sm text-[#8b7355] mt-1">
          Configure which hypercert (impact certificate) holders can access this
          jar
        </p>
      </div>
      ...
      <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        {cert.totalSupply && (
                          <p className="text-xs text-[#8b7355] mt-1">
                            Total Supply:{" "}
                            {Number(cert.totalSupply).toLocaleString()} units
                          </p>
                        )}
                      </div>
                      {cert.metadata?.image && (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-[#3c2a14]">
                    {hypercertInfo.metadata?.name ||
                      `Hypercert #${hypercertInfo.tokenId}`}
                  </p>
                </div>
                <div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="text-xs font-mono text-[#3c2a14] bg-gray-100 px-2 py-1 rounded">
                      {hypercertInfo.contract.slice(0, 10)}...
                      {hypercertInfo.contract.slice(-8)}
                    </code>
                    <Button
                      variant="ghost"
                      size="sm"
                      ...
                      onClick={() =>
                        window.open(
                          `https://etherscan.io/address/${hypercertInfo.contract}`,
                          "_blank",
                        )
                      }
                    >
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <ul className="text-xs text-[#8b7355] space-y-1 list-disc list-inside">
            ...
            <li>
              Hypercerts are divisible ERC-1155 tokens representing verified
              impact
            </li>
          ...
          <div className="mt-3 pt-3 border-t">
            ...
            <p className="text-xs text-[#8b7355]">
              The contract verifies ownership by calling balanceOf() on the
              hypercert contract and checking that the user's balance meets the
              minimum requirement.
            </p>
          </div>
        </div>
